...
public class Registration implements CdsRuntimeConfiguration {
  ...
  public void eventHandlers(CdsRuntimeConfigurer configurer) {
    ...
    var clientProviderFactory = new SDMClientProviderFactory(binding, connectionPool);

    SDMService sdmService = new SDMServiceImpl(clientProviderFactory);
  ...
  private static CdsProperties.ConnectionPool getConnectionPool(CdsEnvironment env) {
    ...
    Duration timeout =
        Duration.ofSeconds(env.getProperty(prefix.formatted("timeout"), Integer.class, 900));
    int maxConnections = env.getProperty(prefix.formatted("maxConnections"), Integer.class, 50);
    logger.debug(
