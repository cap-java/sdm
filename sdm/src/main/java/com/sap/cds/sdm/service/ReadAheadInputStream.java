            ...
            while (totalBytesRead.get() < totalSize) {
              ...
              if (bytesRead > 0) {
                ...
                if (bytesRead < CHUNK_SIZE) {
                  ...
                  logger.debug("preloadChunks - chunk {} trimmed to {} bytes (last chunk)", chunkCount, bytesRead);
                } else {
                  logger.debug("preloadChunks - chunk {} loaded: {} bytes, totalBytesRead: {}", chunkCount, bytesRead, totalBytesRead.get());
                }

                // Ensure last chunk is enqueued
                chunkQueue.put(bufferRef.get());
                logger.debug("preloadChunks - chunk {} enqueued, queueSize: {}", chunkCount, chunkQueue.size());

                // Only mark as last chunk after enqueuing the last chunk
                if (totalBytesRead.get() >= totalSize) {
                  lastChunkLoaded.set(true);
                  logger.info("Last chunk successfully queued and marked. Total chunks: {}, totalBytesRead: {}", chunkCount, totalBytesRead.get());
                  break;
                }
              } else {
