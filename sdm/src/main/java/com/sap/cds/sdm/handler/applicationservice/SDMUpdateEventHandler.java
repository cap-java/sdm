...
public class SDMUpdateEventHandler implements EventHandler {
  ...
  private void doUpdate(CdsUpdateEventContext context, List<CdsData> data) throws IOException {
    ...
    // get the data and get all the filenames, query the attachments for particular up__ID and then
    // check if filenames list is present within the reult list and return those filenames list
    String duplicateFiles = getDuplicateFileNames(data);
    if (duplicateFiles != null) {
      // context.getMessages().error(String.format(SDMConstants.DUPLICATE_FILES_ERROR,
      // duplicateFiles));
  ...
  private void deleteRemovedAttachments(
      List<CdsData> exitingDataList, List<CdsData> updatedDataList, CdsEntity entity) {
  ...
  private List<String> createDocument(
      List<CdsData> data, String jwtToken, CdsUpdateEventContext context, String up__ID)
      throws IOException {
