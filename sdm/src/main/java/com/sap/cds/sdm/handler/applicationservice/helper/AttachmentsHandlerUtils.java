    ...
    try {
      ...
      if (pathParts.length >= 3) {
        ...
        if (targetEntity.getQualifiedName().equals(parentEntity.getQualifiedName())) {
          ...
          // Nested attachment: use the attachmentPath as-is — it already encodes the owning entity
          // (e.g. "AdminService.Sections.attachments"), ensuring uniqueness even when multiple
          // nested entities expose an "attachments" composition under the same direct child.
