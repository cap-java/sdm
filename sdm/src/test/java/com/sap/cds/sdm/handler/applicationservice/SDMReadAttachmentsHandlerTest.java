...
public class SDMReadAttachmentsHandlerTest {
   ...
   @Mock
    ...
    @InjectMocks
    private SDMReadAttachmentsHandler sdmReadAttachmentsHandler;
    ...
    void testModifyCqnForAttachmentsEntity_Success() {
        ...
        CqnSelect select = Select.from(cdsEntity).where(doc -> doc.get("repositoryId").eq(REPOSITORY_ID_KEY));
    ...
    void testModifyCqnForNonAttachmentsEntity() {
        ...
        // Assert
        verify(context).setCqn(select); // Ensure that the original CqnSelect is set without modification
    }
}
