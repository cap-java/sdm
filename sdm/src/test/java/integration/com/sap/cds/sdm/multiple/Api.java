...
public class Api {
  ...
  public List<String> createReference(
      String appUrl,
      String serviceName,
      String entityName,
      String entityID,
        ...
        new Request.Builder()
            .url(
                "https://"
                    ...
                    + "(ID="
                    + entityID
                    + ",IsActiveEntity=false)/references")
            .method("POST", body)
            .addHeader("Content-Type", "application/json")
            .addHeader("Authorization", token)
            .build();
    ...
    try (Response response = httpClient.newCall(postRequest).execute()) {
      if (response.code() != 201) {
        System.out.println("Create reference failed. Error : " + response.body().string());
        throw new IOException("Could not create reference");
      }
      Map<String, Object> responseMap = objectMapper.readValue(response.body().string(), Map.class);
      referenceID = (String) responseMap.get("ID");

      long startTime = System.nanoTime();
      // Upload file content into the empty reference
      RequestBody fileBody = RequestBody.create(file, MediaType.parse("application/octet-stream"));
      Request fileRequest =
          new Request.Builder()
              .url(
      ...
      try (Response fileResponse = httpClient.newCall(fileRequest).execute()) {
        if (fileResponse.code() != 204) {
          String responseBodyString = fileResponse.body().string();
          System.out.println("Create reference failed. Error : " + responseBodyString);
          error = responseBodyString;
          Request request =
              new Request.Builder()
                  .url(
          ...
          try (Response deleteResponse = httpClient.newCall(request).execute()) {
            if (deleteResponse.code() != 204) {
              System.out.println(
                  "Delete reference failed. Error : " + deleteResponse.body().string());
              throw new IOException("Reference was not created and its container was not deleted");
            }
            List<String> createResponse = new ArrayList<>();
            createResponse.add(error);
            return createResponse;
            ...
            System.out.println(
                "Reference was not created and its container was not deleted : " + e);
          }
        }
        long endTime = System.nanoTime(); // Record end time
        double duration = (endTime - startTime) / 1_000_000_000.0;
        System.out.println("Time taken to create(s) : " + duration);
        List<String> createResponse = new ArrayList<>();
        createResponse.add("Reference created");
        createResponse.add(referenceID);
        return createResponse;
      } catch (IOException e) {
        System.out.println("Reference was not created and its container was not deleted : " + e);
      }
    } catch (IOException e) {
      System.out.println("Reference was not created : " + e);
    }
    List<String> createResponse = new ArrayList<>();
    createResponse.add("Reference was not created");
    return createResponse;
  }

  public List<String> createFootnote(
      String appUrl,
      String serviceName,
      String entityName,
      String entityID,
        ...
        new Request.Builder()
            .url(
                "https://"
                    ...
                    + "(ID="
                    + entityID
                    + ",IsActiveEntity=false)/footnotes")
            .method("POST", body)
            .addHeader("Content-Type", "application/json")
            .addHeader("Authorization", token)
            .build();
    ...
    try (Response response = httpClient.newCall(postRequest).execute()) {
      if (response.code() != 201) {
        System.out.println("Create footnote failed. Error : " + response.body().string());
        throw new IOException("Could not create footnote");
      }
      Map<String, Object> responseMap = objectMapper.readValue(response.body().string(), Map.class);
      footnoteID = (String) responseMap.get("ID");

      long startTime = System.nanoTime();
      // Upload file content into the empty footnote
      RequestBody fileBody = RequestBody.create(file, MediaType.parse("application/octet-stream"));
      Request fileRequest =
          new Request.Builder()
              .url(
      ...
      try (Response fileResponse = httpClient.newCall(fileRequest).execute()) {
        if (fileResponse.code() != 204) {
          String responseBodyString = fileResponse.body().string();
          System.out.println("Create footnote failed. Error : " + responseBodyString);
          error = responseBodyString;
          Request request =
              new Request.Builder()
                  .url(
          ...
          try (Response deleteResponse = httpClient.newCall(request).execute()) {
            if (deleteResponse.code() != 204) {
              System.out.println(
                  "Delete footnote failed. Error : " + deleteResponse.body().string());
              throw new IOException("Footnote was not created and its container was not deleted");
            }
            List<String> createResponse = new ArrayList<>();
            createResponse.add(error);
            return createResponse;
          } catch (IOException e) {
            System.out.println("Footnote was not created and its container was not deleted : " + e);
          }
        }
        long endTime = System.nanoTime(); // Record end time
        double duration = (endTime - startTime) / 1_000_000_000.0;
        System.out.println("Time taken to create(s) : " + duration);
        List<String> createResponse = new ArrayList<>();
        createResponse.add("Footnote created");
        createResponse.add(footnoteID);
        return createResponse;
      } catch (IOException e) {
        System.out.println("Footnote was not created and its container was not deleted : " + e);
      }
    } catch (IOException e) {
      System.out.println("Footnote was not created : " + e);
    }
    List<String> createResponse = new ArrayList<>();
    createResponse.add("Footnote was not created");
    return createResponse;
  }
  ...
  public String readReference(
      String appUrl, String serviceName, String entityName, String entityID, String referenceID)
      throws IOException {
    Request request =
        new Request.Builder()
            .url(
                "https://"
                    ...
                    + "(ID="
                    + entityID
                    + ",IsActiveEntity=true)/references(up__ID="
                    + entityID
                    + ",ID="
                    + referenceID
                    + ",IsActiveEntity=true)/content")
    ...
    try {
      Response response = httpClient.newCall(request).execute();
      if (!response.isSuccessful()) {
        System.out.println("Read reference failed. Error : " + response.body().string());
        throw new IOException("Could not read reference");
      }
      return "OK";
    } catch (IOException e) {
      System.out.println("Could not read reference : " + e);
      return "Could not read reference";
    }
  }

  public String readFootnote(
      String appUrl, String serviceName, String entityName, String entityID, String footnoteID)
      throws IOException {
    Request request =
        new Request.Builder()
            .url(
                "https://"
                    ...
                    + "(ID="
                    + entityID
                    + ",IsActiveEntity=true)/footnotes(up__ID="
                    + entityID
                    + ",ID="
                    + footnoteID
                    + ",IsActiveEntity=true)/content")
    ...
    try {
      Response response = httpClient.newCall(request).execute();
      if (!response.isSuccessful()) {
        System.out.println("Read footnote failed. Error : " + response.body().string());
        throw new IOException("Could not read footnote");
      }
      return "OK";
    } catch (IOException e) {
      System.out.println("Could not read footnote : " + e);
      return "Could not read footnote";
    }
  }
  ...
  public String readReferenceDraft(
      String appUrl, String serviceName, String entityName, String entityID, String referenceID)
      throws IOException {
    Request request =
        new Request.Builder()
            .url(
                "https://"
                    ...
                    + "(ID="
                    + entityID
                    + ",IsActiveEntity=false)/references(up__ID="
                    + entityID
                    + ",ID="
                    + referenceID
                    + ",IsActiveEntity=false)/content")
    ...
    try {
      Response response = httpClient.newCall(request).execute();
      if (!response.isSuccessful()) {
        System.out.println("Read draft reference failed. Error : " + response.body().string());
        throw new IOException("Could not read reference draft");
      }
      return "OK";
    } catch (IOException e) {
      System.out.println("Could not read reference draft : " + e);
      return "Could not read reference draft";
    }
  }

  public String readFootnoteDraft(
      String appUrl, String serviceName, String entityName, String entityID, String footnoteID)
      throws IOException {
    Request request =
        new Request.Builder()
            .url(
                "https://"
                    ...
                    + "(ID="
                    + entityID
                    + ",IsActiveEntity=false)/footnotes(up__ID="
                    + entityID
                    + ",ID="
                    + footnoteID
                    + ",IsActiveEntity=false)/content")
    ...
    try {
      Response response = httpClient.newCall(request).execute();
      if (!response.isSuccessful()) {
        System.out.println("Read draft footnote failed. Error : " + response.body().string());
        throw new IOException("Could not read footnote draft");
      }
      return "OK";
    } catch (IOException e) {
      System.out.println("Could not read footnote draft : " + e);
      return "Could not read footnote draft";
    }
  }
  ...
  public String deleteReference(
      String appUrl, String serviceName, String entityID, String referenceID) {
    Request request =
        new Request.Builder()
            .url(
                "https://"
    ...
    try (Response deleteResponse = httpClient.newCall(request).execute()) {
      if (deleteResponse.code() != 204) {
        System.out.println("Delete reference failed. Error : " + deleteResponse.body().string());
        throw new IOException("Reference was not deleted");
      }
      return "Deleted";
    } catch (IOException e) {
      System.out.println("Reference was not deleted : " + e);
      return "Reference was not deleted";
    }
  }

  public String deleteFootnote(
      String appUrl, String serviceName, String entityID, String footnoteID) {
    Request request =
        new Request.Builder()
            .url(
                "https://"
    ...
    try (Response deleteResponse = httpClient.newCall(request).execute()) {
      if (deleteResponse.code() != 204) {
        System.out.println("Delete footnote failed. Error : " + deleteResponse.body().string());
        throw new IOException("Footnote was not deleted");
      }
      return "Deleted";
    } catch (IOException e) {
      System.out.println("Footnote was not deleted : " + e);
      return "Footnote was not deleted";
    }
  }
  ...
  public String renameReference(
      String appUrl, String serviceName, String entityID, String referenceID, String name) {
    MediaType mediaType = MediaType.parse("application/json");
    RequestBody body =
        RequestBody.create(
            mediaType, ByteString.encodeUtf8("{\n    \"fileName\" : \"" + name + "\"\n}"));
    ...
    try (Response renameResponse = httpClient.newCall(request).execute()) {
      if (renameResponse.code() != 200) {
        System.out.println("Rename reference failed. Error : " + renameResponse.body().string());
        throw new IOException("Reference was not renamed");
      }
      return "Renamed";
    } catch (IOException e) {
      System.out.println("Reference was not renamed : " + e);
      return "Reference was not renamed";
    }
  }

  public String renameFootnote(
      String appUrl, String serviceName, String entityID, String footnoteID, String name) {
    MediaType mediaType = MediaType.parse("application/json");
    RequestBody body =
        RequestBody.create(
            mediaType, ByteString.encodeUtf8("{\n    \"fileName\" : \"" + name + "\"\n}"));
    ...
    try (Response renameResponse = httpClient.newCall(request).execute()) {
      if (renameResponse.code() != 200) {
        System.out.println("Rename footnote failed. Error : " + renameResponse.body().string());
        throw new IOException("Footnote was not renamed");
      }
      return "Renamed";
    } catch (IOException e) {
      System.out.println("Footnote was not renamed : " + e);
      return "Footnote was not renamed";
    }
  }

  public String updateSecondaryAttachmentProperty(
      String appUrl,
      String serviceName,
      String entityID,
      String attachmentID,
  ...
  public String updateSecondaryReferenceProperty(
      String appUrl,
      String serviceName,
      String entityID,
      String referenceID,
    ...
    try (Response updateResponse = httpClient.newCall(request).execute()) {
      if (updateResponse.code() != 200) {
        System.out.println(
            "Updating secondary reference property failed. Error : "
                + updateResponse.body().string());
        throw new IOException("Secondary Reference Property was not updated");
      }
      return "Updated";
    } catch (IOException e) {
      System.out.println("Secondary Reference Property was not updated : " + e);
      return "Secondary Reference Property was not updated";
    }
  }
  ...
  public String updateSecondaryFootnoteProperty(
      String appUrl,
      String serviceName,
      String entityID,
      String footnoteID,
    ...
    try (Response updateResponse = httpClient.newCall(request).execute()) {
      if (updateResponse.code() != 200) {
        System.out.println(
            "Updating secondary footnote property failed. Error : "
                + updateResponse.body().string());
        throw new IOException("Secondary Footnote Property was not updated");
      }
      return "Updated";
    } catch (IOException e) {
      System.out.println("Secondary Footnote Property was not updated : " + e);
      return "Secondary Footnote Property was not updated";
    }
  }
  ...
  public String updateInvalidSecondaryAttachmentProperty(
      String appUrl,
      String serviceName,
      String entityID,
      String attachmentID,
  ...
  public String updateInvalidSecondaryReferenceProperty(
      String appUrl,
      String serviceName,
      String entityID,
      String referenceID,
    ...
    try (Response updateResponse = httpClient.newCall(request).execute()) {
      if (updateResponse.code() != 200) {
        System.out.println(
            "Updating reference invalid secondary property failed. Error : "
                + updateResponse.body().string());
        throw new IOException("Invalid secondary property for reference was not updated");
      }
      return "Updated";
    } catch (IOException e) {
      System.out.println("Invalid secondary property for reference was not updated : " + e);
      return "Secondary Property was not updated";
    }
  }

  public String updateInvalidSecondaryFootnoteProperty(
      String appUrl,
      String serviceName,
      String entityID,
      String footnoteID,
    ...
    try (Response updateResponse = httpClient.newCall(request).execute()) {
      if (updateResponse.code() != 200) {
        System.out.println(
            "Updating footnote invalid secondary property failed. Error : "
                + updateResponse.body().string());
        throw new IOException("Invalid secondary property for footnote was not updated");
      }
      return "Updated";
    } catch (IOException e) {
      System.out.println("Invalid secondary property for footnote was not updated : " + e);
      return "Secondary Property was not updated";
    }
  }
  ...
  public Map<String, Object> fetchReferenceMetadata(
      String appUrl, String serviceName, String entityName, String entityID, String referenceID)
      throws IOException {
    // Construct the URL for fetching reference metadata
    String url =
        "https://"
            + appUrl
            + "/odata/v4/"
    ...
    // Make a GET request to fetch the reference metadata
    Request request =
        new Request.Builder().url(url).get().addHeader("Authorization", token).build();
    ...
    try (Response response = httpClient.newCall(request).execute()) {
      if (response.code() != 200) {
        System.out.println("Response code: " + response.code());
        System.out.println("Fetch reference metadata failed. Error: " + response.body().string());
        throw new IOException("Could not fetch reference metadata");
      } else {
        // Parse the JSON response to extract metadata
        return objectMapper.readValue(
            response.body().string(),
  ...
  public Map<String, Object> fetchFootnoteMetadata(
      String appUrl, String serviceName, String entityName, String entityID, String footnoteID)
      throws IOException {
    // Construct the URL for fetching footnote metadata
    String url =
        "https://"
            + appUrl
            + "/odata/v4/"
    ...
    // Make a GET request to fetch the footnote metadata
    Request request =
        new Request.Builder().url(url).get().addHeader("Authorization", token).build();
    ...
    try (Response response = httpClient.newCall(request).execute()) {
      if (response.code() != 200) {
        System.out.println("Response code: " + response.code());
        System.out.println("Fetch footnote metadata failed. Error: " + response.body().string());
        throw new IOException("Could not fetch footnote metadata");
      } else {
        // Parse the JSON response to extract metadata
        return objectMapper.readValue(
            response.body().string(),
