...
# If CMIS_ACCESS_TOKEN env var is already set (passed by the Java test harness),
# skip the HTTP call and reuse it. This avoids one round-trip per script invocation
# when tests pre-fetch the token once and pass it through ProcessBuilder.environment().
get_token() {
...
# Used by the Java test harness to obtain the token once and cache it
# in a static field, then pass it back via CMIS_ACCESS_TOKEN env var
# on all subsequent invocations to skip repeated HTTP round-trips.
# ===========================================================================
action_get_token() {
  get_token
  echo "$ACCESS_TOKEN"
