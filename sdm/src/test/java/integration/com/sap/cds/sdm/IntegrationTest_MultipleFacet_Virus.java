...
class IntegrationTest_MultipleFacet_Virus {
  ...
  private boolean waitForUploadCompletion(
      String entityId, String attachmentId, String facetName, int timeoutSeconds) {
  ...
  void testUploadSingleAttachmentPDF() throws IOException {
    System.out.println("Test (3) : Upload pdf in all facets");
    ClassLoader classLoader = getClass().getClassLoader();
    File file = new File(classLoader.getResource("sample.pdf").getFile());

    String response = api.editEntityDraft(appUrl, entityName, srvpath, entityID);
    assertEquals("Entity in draft mode", response, "Entity should be in draft mode");
    ...
    for (int i = 0; i < facet.length; i++) {
      ...
      response = api.readAttachmentDraft(appUrl, entityName, facet[i], entityID, attachmentID1[i]);
    ...
    for (int i = 0; i < facet.length; i++) {
      response = api.readAttachment(appUrl, entityName, facet[i], entityID, attachmentID1[i]);
  ...
  void testUploadVirusFileInScannedRepo() throws IOException {
    System.out.println(
        "Test (4) : Upload EICAR virus file in all facets — expect virus scan to reject");
      ...
      if (check.contains("malware") || check.contains("potential malware")) {
        ...
        if (response.equals("Saved")) {
          boolean uploadSucceeded =
              waitForUploadCompletion(entityID, attachmentID2[i], facet[i], 120);
          if (!uploadSucceeded) {
            testStatus = true;
          } else {
            fail(
