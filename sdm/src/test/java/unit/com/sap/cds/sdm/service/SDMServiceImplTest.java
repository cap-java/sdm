...
//          .when(
//              () ->
//                  TokenHandler.getHttpClient(any(), any(), any(),
// eq("TECHNICAL_CREDENTIALS_FLOW")))
...
//      when(mockHttpClient.execute(any(HttpGet.class))).thenThrow(new IOException("Network
// error"));
//
...
//      tokenHandlerMockedStatic.when(() ->
// TokenHandler.getTokenFields(token)).thenReturn(expected);
//      when(httpClient.execute(any(HttpGet.class))).thenReturn(response);
//      when(response.getStatusLine()).thenReturn(statusLine);
//      when(statusLine.getStatusCode()).thenReturn(500);
...
//              ServiceException.class,
//              () -> {
//                sdmServiceImpl.getFolderIdByPath(parentId, repositoryId, sdmCredentials,
// jwtToken);
//              });
//      assertEquals(SDMConstants.USER_NOT_AUTHORISED_ERROR, exception.getMessage());
//
...
//      tokenHandlerMockedStatic.when(TokenHandler::getSDMCredentials).thenReturn(sdmCredentials);
//      tokenHandlerMockedStatic
//          .when(() -> TokenHandler.getDITokenUsingAuthorities(sdmCredentials, userEmail,
// subdomain))
...
//          ServiceException.class,
//          () ->
//              sdmServiceImpl.deleteDocument("delete", "123", "user@example.com",
// "test-subdomain"));
//    }
//  }
//
