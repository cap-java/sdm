...
class DBQueryTest {
  ...
  // =====================================================================
  // Tests for updateIsUploadableOnParentEntity (returns long row count)
  // =====================================================================

  @Test
  void testUpdateIsUploadableOnParentEntity_ReturnsRowCount_WhenRowUpdated() {
    // Arrange
    when(mockResult.rowCount()).thenReturn(1L);
    when(mockPersistenceService.run(any(CqnUpdate.class))).thenReturn(mockResult);
    ...
    long updated =
        dbQuery.updateIsUploadableOnParentEntity(
            ...
            "parent-uuid",
            "ID",
            "isAttachmentsUploadable",
            false);

    // Assert
    assertEquals(1L, updated, "Must return the DB row count (1 row updated)");
    verify(mockPersistenceService).run(any(CqnUpdate.class));
  }

  @Test
  void testUpdateIsUploadableOnParentEntity_ReturnsZero_WhenNoRowFound() {
    // Simulate draft row already gone (draftActivate completed before this handler ran)
    ...
    assertEquals(0L, updated, "Must return 0 when no matching row exists");
  }

  @Test
  void testUpdateIsUploadableOnParentEntity_SetsFieldToTrue() {
    when(mockResult.rowCount()).thenReturn(1L);
    when(mockPersistenceService.run(any(CqnUpdate.class))).thenReturn(mockResult);
  ...
  @Test
  void testUpdateIsUploadableOnParentEntity_DifferentFacetFields() {
    // Verifies that any facet field name is accepted (isReferencesUploadable,
    // isFootnotesUploadable)
    when(mockResult.rowCount()).thenReturn(1L);
    when(mockPersistenceService.run(any(CqnUpdate.class))).thenReturn(mockResult);

    dbQuery.updateIsUploadableOnParentEntity(
        mockDraftEntity, mockPersistenceService, "id1", "ID", "isReferencesUploadable", false);
    dbQuery.updateIsUploadableOnParentEntity(
        mockDraftEntity, mockPersistenceService, "id2", "ID", "isFootnotesUploadable", false);
